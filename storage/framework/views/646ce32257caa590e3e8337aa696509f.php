...
<?php $__env->startSection('content'); ?>
            ...
            <div class="col-md-5 col-lg-4 mb-4">
                <div class="card preview-card h-100 border-0 shadow-sm" style="border-radius: 24px;">
                    <div class="card-body p-4 text-center">
                            ...
                            <div class="d-flex justify-content-between align-items-start gap-3 mb-2">
                                <div>
                                    ...
                                    <div id="live-attributes" class="d-none mt-2">
                                        ...
                                        <span class="badge bg-secondary me-1" id="live-age" style="opacity: 0.9"><i class="fas fa-user-clock me-1"></i><span></span></span>
                                        <span class="badge bg-secondary" id="live-hair" style="opacity: 0.9"><i class="fas fa-cut me-1"></i><span></span></span>
                                    </div>
                                </div>
                        ...
                        <div id="kriteria-tambahan" class="mb-4 text-start d-none">
                            <h6 class="fw-bold mb-3 d-none">Kriteria Rekomendasi:</h6>
                            <div class="mb-3">
                                <label class="form-label small text-muted fw-bold">Gender</label>
                                <select id="input-gender" class="form-select" style="border-radius: 12px;">
                                    <option value="male">Pria</option>
                                    <option value="female">Wanita</option>
                                </select>
                            </div>
                            <div class="mb-3">
                                <label class="form-label small text-muted fw-bold">Kelompok Usia</label>
                                <select id="input-age" class="form-select" style="border-radius: 12px;">
                            ...
                            <div class="mb-3">
                                <label class="form-label small text-muted fw-bold">Jenis Rambut</label>
                                <select id="input-hair" class="form-select" style="border-radius: 12px;">
                                    <option value="Lurus">Lurus</option>
                                    <option value="Bergelombang">Bergelombang</option>
                                    <option value="Gimbal">Gimbal</option>
                                    <option value="Keriting">Keriting</option>
                                    <option value="Kribo">Kribo</option>
                                </select>
                            </div>
                        </div>
    ...
    <!-- Scripts -->
    <!-- Face-API.js (Ringan & Cepat < 5MB) menggantikan TensorFlow 95MB untuk face -->
    <script src="https://cdn.jsdelivr.net/npm/@vladmandic/face-api/dist/face-api.min.js"></script>
    <!-- TFJS (diperlukan untuk model klasifikasi rambut) -->
    ...
    <script>
        ...
        function resetResultView() {
            ...
            document.getElementById('kriteria-tambahan').classList.add('d-none');
            ...
            const liveAttr = document.getElementById('live-attributes');
            if (liveAttr) liveAttr.classList.add('d-none');
        }

        let pendingImageSrc = null;
        let isModelLoaded = false;
        let isHairModelLoaded = false;
        ...
        async function loadModels() {
            try {
                ...
                // Hair Model TFJS telah diganti dengan algoritma Image Processing yang jauh lebih cepat & akurat
                isHairModelLoaded = false;
        ...
        async function processImage(src) {
            ...
            // Didefinisikan SEBELUM src diisi untuk menghindari Race Condition
            imgPreview.onload = async () => {
                ...
                // Auto-select gender based on detection
                // Threshold untuk wanita dinaikkan menjadi 0.90 agar pria tidak mudah salah terdeteksi sebagai wanita
                let detectedGender = 'male';
                if (face.gender === 'female' && face.genderProbability > 0.90) {
                ...
                if (tenantKategori === 'salon' && detectedGender === 'male') {
                    showError('Akses Ditolak', 'Mohon maaf, Salon ini khusus melayani pelanggan wanita.');
                    return;
                }

                document.getElementById('input-gender').value = detectedGender;
                
                // Auto-select age group (Hanya Remaja dan Dewasa sesuai klasifikasi)
                if (face.age) {
                    if (face.age < 20) {
                        document.getElementById('input-age').value = 'teen';
                    } else {
                        document.getElementById('input-age').value = 'adult';
                    }
                }

                // Analisis Tekstur Rambut (Heuristik Edge Density) sebagai pengganti model AI
                let predictedHairType = 'Lurus'; // Default
                try {
                    const box = face.detection.box;
                    const canvas = document.createElement('canvas');
                    const ctx = canvas.getContext('2d');
                    
                    // Ambil porsi rambut di atas wajah (lebar dilebihkan, tinggi dari dahi ke atas)
                    const hairY = Math.max(0, box.y - box.height * 0.4); 
                    const hairHeight = Math.max(10, box.y + box.height * 0.1 - hairY);
                    const hairX = Math.max(0, box.x - box.width * 0.2);
                    const hairWidth = Math.min(imgPreview.width - hairX, box.width * 1.4);
                    
                    canvas.width = hairWidth;
                    canvas.height = hairHeight;
                    ctx.drawImage(imgPreview, hairX, hairY, hairWidth, hairHeight, 0, 0, hairWidth, hairHeight);
                    
                    const imgData = ctx.getImageData(0, 0, hairWidth, hairHeight);
                    const data = imgData.data;
                    const gray = new Uint8Array(hairWidth * hairHeight);
                    for (let i = 0; i < data.length; i += 4) {
                        gray[i/4] = 0.299 * data[i] + 0.587 * data[i+1] + 0.114 * data[i+2]; // Konversi Grayscale
                    }
                    
                    let edgePixels = 0;
                    const threshold = 18; // Sensitivitas garis tekstur
                    
                    for (let y = 0; y < hairHeight - 1; y++) {
                        for (let x = 0; x < hairWidth - 1; x++) {
                            const idx = Math.floor(y) * Math.floor(hairWidth) + Math.floor(x);
                            const diffX = Math.abs(gray[idx] - gray[idx + 1]);
                            const diffY = Math.abs(gray[idx] - gray[idx + Math.floor(hairWidth)]);
                    ...
                    const density = edgePixels / (hairWidth * hairHeight);
                    console.log("Kepadatan Tekstur Rambut (Edge Density):", density);
                    
                    if (density > 0.35) {
                        predictedHairType = 'Kribo';
                    } else if (density > 0.22) {
                        predictedHairType = 'Keriting';
                    } else if (density > 0.14) {
                        predictedHairType = 'Gimbal';
                    } else if (density > 0.08) {
                        predictedHairType = 'Bergelombang';
                    } else {
                        predictedHairType = 'Lurus';
                    }
                } catch (e) {
                    console.error("Gagal mendeteksi tekstur:", e);
                }
                
                document.getElementById('analysis-caption').innerText = "Rambut terdeteksi sebagai: " + predictedHairType;
                document.getElementById('input-hair').value = predictedHairType;
                ...
                document.getElementById('analysis-caption').innerText = 'Wajah dan atribut berhasil diklasifikasikan. Silakan tekan tombol di bawah untuk melihat hasil.';
                
                const translatedGender = document.getElementById('input-gender').value === 'male' ? 'Pria' : 'Wanita';
                const translatedAge = document.getElementById('input-age').value === 'teen' ? 'Remaja' : 'Dewasa';
                const predictedHair = document.getElementById('input-hair').value;

                document.querySelector('#live-gender span').innerText = translatedGender;
                document.querySelector('#live-age span').innerText = translatedAge;
                document.querySelector('#live-hair span').innerText = predictedHair;
                document.getElementById('live-attributes').classList.remove('d-none');

                // Kriteria tambahan tidak ditampilkan karena klasifikasi berjalan otomatis
                document.getElementById('kriteria-tambahan').classList.add('d-none');
                btnKirim.disabled = false;
                
                } catch (generalError) {
                    console.error("Critical Error in processImage:", generalError);
                    document.getElementById('analysis-caption').innerHTML = `<span class="text-danger">Error Sistem: ${generalError.message}</span>`;
                }
            };
            
            // Set sumber gambar setelah event handler terpasang!
            imgPreview.src = src;
        }
        ...
        function kirimHasil() {
            ...
            fetch("<?php echo e(route('rekomendasi.process')); ?>", {
                    ...
                    body: JSON.stringify({
                        ...
                        hair_type: document.getElementById('input-hair').value
                    })
                })
                .then(res => res.json())
                .then(data => {
