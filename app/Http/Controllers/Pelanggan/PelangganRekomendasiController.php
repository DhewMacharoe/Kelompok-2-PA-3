...
class PelangganRekomendasiController extends Controller
{
    ...
    public function process(Request $request)
    {
        $request->validate([
            ...
            'jenis_rambut' => 'nullable|string'
        ]);

        $bentukWajah = ucfirst(strtolower($request->input('bentuk_wajah')));
        $akurasi = $request->input('akurasi_sistem');
        $gender = strtolower($request->input('gender', 'male'));
        $age = (int) $request->input('age', 25);
        $jenisRambut = ucfirst(strtolower($request->input('jenis_rambut', 'Lurus')));
        ...
        $jsonPath = storage_path('app/rekomendasi_rules.json');
        if (file_exists($jsonPath)) {
            $rules = json_decode(file_get_contents($jsonPath), true);
            if (isset($rules[$gender][$ageCategory][$bentukWajah][$jenisRambut])) {
                $faceProfile = $rules[$gender][$ageCategory][$bentukWajah][$jenisRambut];
            }
        }
