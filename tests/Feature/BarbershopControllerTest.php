...
class BarbershopControllerTest extends TestCase
{
    ...
    public function test_admin_can_view_barbershop_settings(): void
    {
        $response = $this->actingAs($this->admin)
            ->get(route('admin.barbershop.index'));
    ...
    public function test_admin_can_view_edit_barbershop_page_with_map_fields(): void
    {
        $response = $this->actingAs($this->admin)
            ->get(route('admin.barbershop.edit', $this->barbershop->id));
    ...
    public function test_admin_can_update_barbershop_settings_including_maps(): void
    {
        $response = $this->actingAs($this->admin)
            ->put(route('admin.barbershop.update', $this->barbershop->id), [
        ...
        $this->barbershop->refresh();
        $this->assertEquals('New Brand Name', $this->barbershop->nama_brand);
        $this->assertEquals('New Hero Description Text', $this->barbershop->deskripsi_hero);
        $this->assertEquals('New Layanan Hero Title', $this->barbershop->judul_hero_layanan);
        $this->assertEquals('New Layanan Hero Desc', $this->barbershop->deskripsi_hero_layanan);
        $this->assertEquals('https://www.google.com/maps/search/?api=1&query=2.456,99.789', $this->barbershop->kontak['link_map']);
        $this->assertEquals('https://maps.google.com/maps?q=2.456,99.789&z=15&output=embed', $this->barbershop->kontak['map_embed']);
    }

    public function test_admin_can_update_barbershop_settings_with_hero_image(): void
    {
        ...
        $this->barbershop->refresh();
        $this->assertNotNull($this->barbershop->gambar_hero);
        $this->assertFileExists(public_path($this->barbershop->gambar_hero));
        $this->assertNotNull($this->barbershop->gambar_hero_layanan);
        $this->assertFileExists(public_path($this->barbershop->gambar_hero_layanan));

        // Clean up the uploaded fake files
        if (file_exists(public_path($this->barbershop->gambar_hero))) {
            @unlink(public_path($this->barbershop->gambar_hero));
        }
        if (file_exists(public_path($this->barbershop->gambar_hero_layanan))) {
            @unlink(public_path($this->barbershop->gambar_hero_layanan));
        }
    }

    public function test_reset_location_in_lokasi_page_uses_active_barbershop_map_link(): void
    {
        // Set a custom link map on active barbershop
        $this->barbershop->update([
            'kontak' => array_merge($this->barbershop->kontak, [
        ...
        $response->assertStatus(200);
        // Verify it includes the active barbershop's link in the JS response
